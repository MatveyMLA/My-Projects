(function() {
	...
	companyModule.service('CompanyService', function($http) {
		...
		this.createCompany = function(data) {
			...
			$http.post('http://localhost:8080/Coupons/rest/companies/', data)			
		...
		this.updateCompany = function(data) {
			...
			$http.put('http://localhost:8080/Coupons/rest/companies/', data)			
			.success(function(response) {
				alert(response);
			})
			.error(function(reponse) {
		...
		this.getCompany = function(data) {
			...
			return $http.get('http://localhost:8080/Coupons/rest/companies/' + data)			
			.success(function(response) {
				return response;
			})
			.error(function(reponse) {
		...
		this.removeCompany = function(data) {
			$http.delete('http://localhost:8080/Coupons/rest/companies/' + data)			
			.success(function(response) {
			})
			.error(function(reponse) {
				alert("ERROR!");
		...
		this.getAllCompanies = function() {
			...
			return $http.get('http://localhost:8080/Coupons/rest/companies/allcompanies')			
			.success(function(response) {
				return response;
			})
			.error(function(reponse) {
